      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[#E8F5E9]/60 via-[#F8FAF8] to-[#F8FAF8] pt-12 pb-20 md:pt-20 md:pb-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            {/* Visual Hero Feature Card with Product Image */}
            <div className="relative">
              {(() => {
                const spotlightProduct = featuredProducts.find(
                  (p) => p.name?.toLowerCase().includes('bamboo cutlery') || p.slug === 'bamboo-cutlery-straw-dining-kit'
                );
                const spotlightImage = spotlightProduct?.images?.[0] || '/products/product-01.jpg';

                return (
                  <div className="w-full rounded-3xl bg-gradient-to-tr from-[#1B4332] to-[#2E7D32] p-6 sm:p-8 text-white flex flex-col justify-between shadow-xl relative overflow-hidden group min-h-[380px] sm:min-h-[420px]">
                    ...
                    {/* Middle Section: Text on Left, Product Image on Right */}
                    <div className="relative z-10 my-4 grid grid-cols-1 sm:grid-cols-12 gap-6 items-center">
                      ...
                      <div className="sm:col-span-7 space-y-2">
                        ...
                        <h3 className="text-xl sm:text-2xl font-bold leading-tight">
                          {spotlightProduct?.name || 'Bamboo Cutlery & Straw Dining Kit'}
                        </h3>
                        <p className="text-xs text-emerald-100 leading-relaxed max-w-sm">
                          {spotlightProduct?.shortDescription || spotlightProduct?.description || 'Handcrafted organic bamboo cutlery kit designed for zero-waste dining on the go.'}
                        </p>
                      </div>

                      {/* Right Product Image Container */}
                      <div className="sm:col-span-5 flex items-center justify-center sm:justify-end">
                        <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 p-2.5 flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform duration-300 shrink-0">
                          <img
                            src={spotlightImage}
                            alt={spotlightProduct?.name || 'Bamboo Cutlery & Straw Dining Kit'}
                            onError={(e) => {
                    ...
                    <div className="relative z-10 flex items-center justify-between pt-4 border-t border-white/20">
                      <div>
                        ...
                        <span className="text-xl font-bold">₹{spotlightProduct?.price || 899}</span>
                      </div>
                      <Link
                        to={spotlightProduct ? `/product/${spotlightProduct.slug || spotlightProduct._id}` : '/shop?category=Reusable'}
